...
class GameTest {
    ...
    void canPlaceRespectsPatternsAndOccupancy() {
        ...
        // Occupy (2,2)
        g.makeMove(new Move(new Point(2,2), Stone.BLACK));
        assertFalse(g.canPlace(new Point(2,2), Stone.WHITE), "occupied cell");

        // Set up near-X and check pattern denial
        g.makeMove(new Move(new Point(3,2), Stone.WHITE));
        g.makeMove(new Move(new Point(2,3), Stone.WHITE));
    ...
    void placeAndWin() {
        ...
        g.makeMove(new Move(new Point(0,1), Stone.WHITE));
        g.makeMove(new Move(new Point(1,1), Stone.WHITE));
        g.makeMove(new Move(new Point(2,1), Stone.WHITE));
